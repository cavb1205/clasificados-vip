...
export default function HostPanelPage() {
  ...
  const loadRooms = useCallback(async () => {
    setListings(await rooms.myRooms());
  }, []);
    ...
    <div className="mx-auto max-w-3xl space-y-8">
      ...
      {!host ? (
        ...
        <>
          ...
          <NewRoomForm plans={plans} host={host} onCreated={loadRooms} />
          <section className="space-y-4">
            ...
            {listings.length === 0 ? (
              <p className="text-sm text-neutral-500">
                Aún no tienes habitaciones publicadas.
              </p>
            ) : (
              listings.map((r) => (
                <RoomCard key={r.id} room={r} plans={plans} onChange={loadRooms} />
              ))
            )}
          </section>
        </>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <button
              disabled={busy}
              onClick={save}
              className="rounded-full bg-pink-600 px-4 py-1.5 text-xs font-medium hover:bg-pink-500 disabled:opacity-50"
            >
              Guardar
            </button>
            <button
              onClick={() => setEditing(false)}
              className="rounded-full border border-neutral-700 px-4 py-1.5 text-xs"
            >
              Cancelar
            </button>
          </div>
...
function NewRoomForm({
  ...
  useEffect(() => {
    if (open && regions.length === 0) {
      dashboard.regions().then((r) => setRegions(r as Region[]));
    }
  }, [open, regions.length]);

  useEffect(() => {
    if (form.region) {
      dashboard.cities(form.region).then((c) => setCities(c as City[]));
    } else {
      setCities([]);
    }
  }, [form.region]);
  ...
  if (!open) {
    return (
      <button
        ...
        className="rounded-full bg-pink-600 px-5 py-2 text-sm font-medium hover:bg-pink-500"
      >
        + Publicar habitación
      </button>
    );
  ...
  return (
    ...
    >
      ...
      <input
        required
        placeholder="Título (ej: Pieza amoblada, sector centro)"
        className={inputCls}
        value={form.title}
        onChange={(e) => setForm({ ...form, title: e.target.value })}
      />
      <textarea
        placeholder="Descripción (servicios, condiciones…)"
        className={inputCls}
        rows={3}
      ...
      <div className="grid grid-cols-2 gap-3">
        <select
          ...
          className={inputCls}
          value={form.region}
          onChange={(e) => setForm({ ...form, region: e.target.value, city_id: "" })}
        >
      ...
      <input
        placeholder="Sector (referencia general, sin dirección exacta)"
        className={inputCls}
        value={form.sector}
      ...
      {plans.length === 0 && (
        <p className="text-xs text-amber-400">
          No hay planes de habitación configurados todavía. Podrás crear el anuncio en
          borrador y pagarlo cuando el administrador habilite un plan.
        </p>
      )}
      {err && <p className="text-sm text-red-400">{err}</p>}
      <div className="flex gap-2">
        ...
        <button
          ...
          className="rounded-full border border-neutral-700 px-5 py-2 text-sm"
        >
          Cancelar
        </button>
      </div>
...
function RoomCard({
  ...
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState("");
  const [planId, setPlanId] = useState<string>(room.plan ? String(room.plan.id) : "");
  ...
  async function onReceipt(e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0];
    if (!file) return;
    if (planId) await rooms.updateRoom(room.id, { plan_id: Number(planId) });
    const fd = new FormData();
    fd.append("image", file);
    await run(() => rooms.uploadRoomReceipt(room.id, fd));
    if (receiptInput.current) receiptInput.current.value = "";
  }

  const live = room.status === "active" && !room.is_paused;
    ...
    <article className="rounded-xl border border-neutral-800 bg-neutral-900 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-neutral-400">
            {room.city ?? "—"}
            {room.sector && ` · ${room.sector}`} · {CLP.format(room.price)}{" "}
            {PERIOD_LABEL[room.price_period]}
          </p>
          <p className="mt-1 text-xs">
            ...
            {room.expires_at && (
              <span className="ml-2 text-neutral-500">
                vence {new Date(room.expires_at).toLocaleDateString("es-CL")}
              </span>
            )}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          {room.status === "active" &&
            (room.is_paused ? (
              <button
                disabled={busy}
                onClick={() => run(() => rooms.resumeRoom(room.id))}
              ...
              <button
                disabled={busy}
                onClick={() => run(() => rooms.pauseRoom(room.id))}
                className="rounded-full border border-amber-500 px-3 py-1.5 text-xs text-amber-300 hover:bg-amber-600/20 disabled:opacity-50"
              >
                Pausar (ocupada)
              </button>
            ))}
          <button
            disabled={busy}
            onClick={() => {
              if (confirm("¿Eliminar esta habitación?")) run(() => rooms.deleteRoom(room.id));
            }}
        ...
        <div className="mt-3 flex gap-2 overflow-x-auto">
          {room.photos.map((ph) => (
            <div key={ph.id} className="relative">
              ...
              />
              <button
                onClick={() => run(() => rooms.deleteRoomPhoto(ph.id))}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <label
          ...
          className="cursor-pointer rounded-full border border-neutral-700 px-3 py-1.5 text-xs hover:border-pink-500"
        >
          + Agregar foto
        </label>

        {/* Pago: solo cuando es borrador o expirada */}
        {(room.status === "draft" || room.status === "expired") && (
          <>
            <select
              className="rounded-lg border border-neutral-700 bg-neutral-900 px-3 py-1.5 text-xs"
              value={planId}
            ...
            >
              ...
              {plans.map((p) => (
                <option key={p.id} value={p.id}>
                  {p.name} · {CLP.format(p.price)} · {p.duration_days}d
                </option>
              ))}
            </select>
            <input
            ...
            />
            <label
              htmlFor={`receipt-${room.id}`}
              className={`rounded-full px-3 py-1.5 text-xs font-medium ${
                planId
                  ? "cursor-pointer bg-pink-600 hover:bg-pink-500"
