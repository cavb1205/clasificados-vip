...
class HostProfileAdmin(admin.ModelAdmin):
    list_display = ("display_name", "user", "active_plan", "plan_slots",
...
class RoomListingAdmin(admin.ModelAdmin):
    list_display = ("title", "owner", "city", "price", "price_period", "status",
                    "is_featured", "is_paused", "is_suspended", "expires_at")
    list_filter = ("status", "price_period", "is_featured", "is_paused", "is_suspended")
